...
@pytest.mark.asyncio
async def test_set_gas_limit_202(httpx_mock: HTTPXMock):
    ...
    response = await client.set_gas_limit.asyncio_detailed(pubkey=pubkey, gas_limit="999999")
...
async def test_set_gas_limit_errors(status_code: int, mock_response: Dict, httpx_mock: HTTPXMock):
    ...
    # .asyncio method returns None
    response = await client.set_gas_limit.asyncio(pubkey=pubkey, gas_limit="999999")
...
@pytest.mark.asyncio
async def test_get_gas_limit_200(httpx_mock: HTTPXMock):
    ...
    response = await client.get_gas_limit.asyncio(pubkey=pubkey)
...
@pytest.mark.asyncio
@pytest.mark.parametrize("status_code, mock_response", test_data_error_responses)
async def test_get_gas_limit_errors(status_code: int, mock_response: Dict, httpx_mock: HTTPXMock):
...
@pytest.mark.asyncio
async def test_delete_gas_limit_204(httpx_mock: HTTPXMock):
    ...
    response = await client.delete_gas_limit.asyncio_detailed(pubkey=pubkey)
...
@pytest.mark.asyncio
@pytest.mark.parametrize("status_code, mock_response", test_data_error_responses)
async def test_delete_gas_limit_errors(status_code: int, mock_response: Dict, httpx_mock: HTTPXMock):
    ...
    response = await client.delete_gas_limit.asyncio(pubkey=pubkey)
