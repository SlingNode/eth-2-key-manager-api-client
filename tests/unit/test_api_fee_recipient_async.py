...
@pytest.mark.asyncio
async def test_set_fee_recipient_202(httpx_mock: HTTPXMock):
    ...
    response = await client.set_fee_recipient.asyncio_detailed(pubkey=pubkey, ethaddress="0xabcf8e0d4e9587369b2301d0790347320302cc09")
...
async def test_set_fee_recipient_errors(status_code: int, mock_response: Dict, httpx_mock: HTTPXMock):
    ...
    response = await client.set_fee_recipient.asyncio(pubkey=pubkey, ethaddress="0xabcf8e0d4e9587369b2301d0790347320302cc09")
...
@pytest.mark.asyncio
async def test_list_fee_recipient_200(httpx_mock: HTTPXMock):
    ...
    response = await client.list_fee_recipient.asyncio(pubkey=pubkey)
...
async def test_list_fee_recipient_errors(status_code: int, mock_response: Dict, httpx_mock: HTTPXMock):
...
@pytest.mark.asyncio
async def test_delete_fee_recipient_204(httpx_mock: HTTPXMock):
    ...
    response = await client.delete_fee_recipient.asyncio_detailed(pubkey=pubkey)
...
@pytest.mark.asyncio
@pytest.mark.parametrize("status_code, mock_response", test_data_error_responses)
async def test_delete_fee_recipient_errors(status_code: int, mock_response: Dict, httpx_mock: HTTPXMock):
    ...
    response = await client.delete_fee_recipient.asyncio(pubkey=pubkey)
