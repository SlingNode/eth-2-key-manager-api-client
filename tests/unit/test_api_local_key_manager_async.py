...
@pytest.mark.asyncio
async def test_list_keys_200(httpx_mock: HTTPXMock):
    ...
    response = await client.list_keys.asyncio()
...
async def test_list_keys_errors(status_code: int, mock_response: Dict, httpx_mock: HTTPXMock):
...
@pytest.mark.asyncio
async def test_unexpected_status_code_raise_on_unexpected_status(httpx_mock: HTTPXMock):
    ...
    with pytest.raises(errors.UnexpectedStatus):
        await client.list_keys.asyncio()


@pytest.mark.asyncio
async def test_unexpected_status_code(httpx_mock: HTTPXMock):
    """Test case for list_keys with unexpected status code"""

    httpx_mock.add_response(status_code=407, json={"message": "Unexpected status code"})
    client = eth_2_key_manager_api_client.Eth2KeyManager(base_url="http://localhost:8080", token="test_token")
    response = await client.list_keys.asyncio_detailed()
...
@pytest.mark.asyncio
async def test_import_keystores_200(httpx_mock: HTTPXMock, keystore_str: str, keystore_password_str: str, slashing_protection_str: str):
    ...
    client = eth_2_key_manager_api_client.Eth2KeyManager(base_url="http://localhost:8080", token="test_token")
    response = await client.import_keystores.asyncio([keystore_str], [keystore_password_str], slashing_protection_str)
...
@pytest.mark.asyncio
async def test_import_keystores_200_without_slashing_protection(httpx_mock: HTTPXMock, keystore_str: str, keystore_password_str: str):
    """Test case for import_keystores with 200 response"""
    ...
    client = eth_2_key_manager_api_client.Eth2KeyManager(base_url="http://localhost:8080", token="test_token")
    response = await client.import_keystores.asyncio([keystore_str], [keystore_password_str])
...
async def test_import_keystores_errors(status_code: int, mock_response: Dict, httpx_mock: HTTPXMock, keystore_str: str, keystore_password_str):
    """Test case for import_keystores errors"""

    httpx_mock.add_response(status_code=status_code, json=mock_response)

    client = eth_2_key_manager_api_client.Eth2KeyManager(base_url="http://localhost:8080", token="test_token")
    response = await client.import_keystores.asyncio([keystore_str], [keystore_password_str], "invalid_slashing_protection")
...
@pytest.mark.asyncio
async def test_delete_keys_200(httpx_mock: HTTPXMock):
    ...
    client = eth_2_key_manager_api_client.Eth2KeyManager(base_url="http://localhost:8080", token="test_token")
    response = await client.delete_keys.asyncio([pubkey])
...
@pytest.mark.asyncio
@pytest.mark.parametrize("status_code, mock_response", test_data_error_responses + [(400, mock_response_400)])
async def test_delete_keys_errors(status_code: int, mock_response: Dict, httpx_mock: HTTPXMock):
